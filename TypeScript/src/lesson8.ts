...
const stringEcho = (arg: string): string => arg // for strings only
const echo = <T>(arg: T): T => arg // Generic, for all datatypes
...
const isTrue = <T>(arg: T): {arg: T, is: boolean} => {
    ...
    if (isObj(arg) && !Object.keys(arg as keyof T).length){ // if empty object (such as string)
        return {arg, is: false}  
    }
...
const isTrue1 = <T>(arg: T): BoolCheck<T> => { // // same function using the interface BoolCheck
    if (Array.isArray(arg) && !arg.length){
...
const processUser = <T extends HasID>(user: T): T => { // type has to have the ID property
